...
/// - size_t is typically used for sizes, prefer int32 for indexing unless you wish to avoid an implicity type conversion
/// from STL containers
...
/// - If a built-in function uses unspecified int type / long type, implict/explicit casting to the built-in function is
/// allowed
...
///
/// Notes on "Byte":
/// - When on a 32/64 bit system it is safe to assume a byte is 8 bits and thus terminology like numBytes is synonymous with
/// number
///   of int8_t/uint8_t values and it is safe for byte dependent operations
/// - If not, then it is important to take care of using int8/uint8 interchangeably with a byte/char/uchar types and byte
/// dependent
///   operations.
///   - E.g using memcpy to copy a buffer of bytes into a network buffer:
///         In this scenario, if the host byte is >8 bits in length then memcopying the bytes directly into the network
///         buffer leaves padding which is incorrect. You would have to write into the buffer manually to eliminate such
///         padding and ensure a buffer of uint8_t for networking.
...
///
/// Guarantees:
/// - float16 is exactly 16 bits -> NOTE(randomuserhi) Interpretation is not built-in due to most systems not supporting 16
/// bit floats
