...
internal class rSteamClient : IDisposable {
    ...
    public rSteamClient(SteamNetworkingIPAddr address, SteamNetworkingConfigValue_t[] options = null, string debugName = "Client") {
        this.debugName = debugName;

        identity = default;

        connection = SteamNetworkingSockets.ConnectByIPAddress(ref address, options == null ? 0 : options.Length, options);
        localClients.Add(connection);
    ...
    private void OnConnectionStatusChanged(SteamNetConnectionStatusChangedCallback_t callbackData) {
        ...
        case ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_Connecting:
            Debug.Log($"[{debugName}] {connection.m_HSteamNetConnection} Connecting to: {connectionInfo.m_szConnectionDescription}");
            connected = false;
            break;

        case ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_Connected:
            Debug.Log($"[{debugName}] {connection.m_HSteamNetConnection} Connection established: {connectionInfo.m_szConnectionDescription}");
        ...
        case ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_ProblemDetectedLocally:
            Debug.Log($"[{debugName}] {connection.m_HSteamNetConnection} Connection closed: {connectionInfo.m_szEndDebug}, established: {connected}");
